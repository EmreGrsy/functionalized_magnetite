...
df = pd.read_csv("45L_full_info.data", header=None, delimiter=" ")
...
for mc_temp in range(len(df.columns)-1):
    ...
    axs.set_title("Tet-full / 21L + 24L / Bulk", fontsize=24)

    axs.set_ylabel(r"Fe$_\mathrm{oct}$ layer", ha="center", fontsize=24, labelpad=10)
    axs.set_xlabel(r"$n_\mathrm{Fe_{oct}}^{3+}$/$n_\mathrm{Fe_{oct}}$", ha="center", fontsize=24, labelpad=10)
